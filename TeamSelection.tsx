    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="w-full h-full flex flex-col py-6">
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-4">
          <button onClick={onBack} className="p-4 bg-white/10 backdrop-blur-md border border-white/20 rounded-full text-[#3C3C3C]">
            <ArrowLeft size={20} />
          </button>
        ...
        <button 
          ...
          disabled={!isReady}
          className="bg-[#00A49E] text-white px-8 py-4 rounded-2xl font-black text-xs uppercase tracking-widest disabled:opacity-20 transition-all shadow-xl"
        >
          Confirm Teams
        </button>
      </div>
      ...
      <AnimatePresence>
        {showInfo && (
          <motion.div 
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[500] bg-[#3C3C3C]/90 backdrop-blur-sm flex items-center justify-center p-8"
          ...
          >
            ...
            >
              ...
              <button 
                onClick={() => setShowInfo(false)}
                className="mt-10 w-full bg-[#00A49E] text-white py-5 rounded-2xl font-black text-sm uppercase tracking-widest shadow-lg hover:brightness-110 transition-all"
              >
                Got it
              </button>
            </motion.div>
      ...
      <div className="flex-1 flex flex-col gap-12">
        {/* Unassigned Pool */}
        <div className="bg-white/10 backdrop-blur-xl rounded-[2.5rem] p-10 border border-white/20 shadow-2xl min-h-[200px]">
          ...
          <div className="flex flex-wrap gap-4 justify-center">
            {unassigned.map(p => (
              <motion.div
                ...
                className="bg-white/10 backdrop-blur-md px-6 py-4 rounded-2xl shadow-xl border border-white/20 cursor-grab active:cursor-grabbing flex items-center gap-3"
              >
        ...
        <div className="grid grid-cols-3 gap-8 flex-1">
          {buckets.map(b => (
            ...
            >
              <div className="w-12 h-12 rounded-full mb-4 shadow-xl flex items-center justify-center border-2 border-white/20" style={{ backgroundColor: b.color }}>
                ...
                <AnimatePresence>
                  {assignedPlayers.filter(p => p.teamId === b.id).map(p => (
                    <motion.div
                      ...
                      className="bg-white/10 backdrop-blur-md p-3 rounded-xl shadow-sm border border-white/10 flex items-center justify-between group"
                    >
